...
- „Wie erstelle ich Features aus einem Timestamp: `hour`, `weekday`, `is_weekend`?“
...
- „Wie mache ich einen Vergleich Ankunft vs Abfahrt, zwei Verteilungen in einem Plot, gleiche Skala?“
- „Wie ändere ich Plot-Farben/Theme so, dass es report-tauglich aussieht?“
...
- „Wie visualisiere ich Unterschiede zwischen Betreibern/Operatoren (Boxplot je Betreiber) und verhindere, dass es durch viele Kategorien unlesbar wird?“
...
- „Was ist das Simpson-Paradoxon und wie lässt es sich anhand unseres Datensatzes veranschaulichen?“
...
- „Wie berechne ich ein Konfidenzintervall für einen **Anteil** (z.B. Pünktlichkeit) und welche Methode ist sinnvoll?“
...
- „Ich habe `BPUIC` in den Verspätungsdaten und in `dienststellen.csv` gibt es `number` / `municipalityName` / `cantonAbbreviation`. Wie mappe ich `BPUIC` → Gemeinde/Kanton sauber?“
...
- „Ich habe Gemeindenamen aus SBB-Stationen, aber sie stimmen nicht exakt mit den BFS-Gemeinden in der GeoPackage-Datei überein. Wie mache ich diese korrekt matchen mit Kanton-Einschränkung, um False Matches zu vermeiden?“
...
- „Ich möchte eine **Folium Choropleth/GeoJson-Karte** für Ø Verspätung pro Gemeinde erstellen. Wie mache ich das mit GeoPandas → WGS84 und Folium?“
- „Mein GPKG ist in 3d und die Z-koordinate ist zu viel und Folium meckert. Wie entferne ich Z-Koordinaten?“
...
- „Wie ändere ich die Farben/Opacity/Border-Lines in Folium, damit es besser aussieht?“
...
- „Wie rotiere ich Kanton-Labels, setze eine Null-Linie, und mache den Plot report-tauglich?“
...
- „Ich möchte `delay_min` modellieren über `hour` und `weekday`. Wie baue ich ein OLS-Modell?“
- „Wie füge ich **polynomiale Terme** hinzu und interpretiere die Nichtlinearität?“
- „Welche Transformation ist sinnvoll bei Delay, und wie setze ich das in der Formel um?“
...
- „Wie baue ich ein OLS-Modell mit Interaktion **hour × weekday** in statsmodels?“
...
- „Wie fitten wir `PanelOLS` mit `EntityEffects` und **clustered SE** auf entity-level?“
...
- „In meinem IST-Datensatz ist die Bahnhof-ID `BPUIC`. In `dienststellen.csv` scheint `operatingPointKilometerMasterNumber` dazu zu passen. Wie joine ich das sauber?“
...
- „Wie kann ich die Karten als PNG speichern ohne abgeschnittene Labels?“
- „Wie ändere ich Farben/Alpha/edgecolors, damit der Plot besser aussieht?“
...
- „Wie verhindere ich, dass die Tagesplots zu langsam werden?“
- „Wie mache ich die Titel dynamisch (z.B. ‘Verspätungsentwicklung am YYYY-MM-DD’)?“
...
- „Wie untersuche ich den Zusammenhang zwischen `mean_delta_delay` und `anzahl_zuege` mit einem Regressionsansatz, und welche Diagnose- und Interpretationspunkte sollte ich dabei beachten?“
- „Wir sehen in der Schweiz kaum Unterschiede zwischen Bahnhöfen. Wann ist der Efefekt schwach oder nicht robust?“